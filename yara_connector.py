...
class YaraConnector(BaseConnector):
    ...
    def _fetch_yara_source(self, location: str = "") -> None:
        ...
        if response.headers.get("Content-Type", "").find("text/plain") > -1:
            ...
            with open(PurePath(self.state_dir, location.split("/")[-1].split("?")[0]), "wb") as filename:
    ...
    def _handle_update_yara_sources(self) -> RetVal:
        ...
        if env_vars:
            ...
            message = "No asset environment variables configured.  Add URL sources to fetch!"
            return self._return_with_message(message, action_result, APP_ERROR)
        ...
        for file in Path(PurePath(self.state_dir, param.get("path", ""))).iterdir():
            if not file.name.endswith("state.json"):
                try:
                    ...
                    return self._return_with_message(f"Failed removing files -- {error}", action_result, APP_ERROR)
        ...
        for file in Path(self.state_dir).glob(extension):
            if not file.name.endswith("state.json"):
                ...
                action_result.add_data(
                    {
                        **{"name": str(Path(file))},
                        **{key: getattr(file_info, key) for key in dir(file_info) if key.startswith("st_")},
                    }
                )
    ...
    def _handle_yara_scan(self, param) -> RetVal:
        ...
        action_result = self.add_action_result(ActionResult(dict(param)))
        self.save_progress(f"In action handler for: {self.get_action_identifier()}")
        cached_ruleset = hashlib.blake2b(f"{param.get('yara_path', '**')}".encode()).hexdigest()
        try:
            ...
            try:
                rules = yara.compile(
                    filepaths={file.name: str(file) for file in Path(self.state_dir).glob(param.get("yara_path", "**/*.yar"))},
        ...
        scan_location: str = param.get("scan_dir", vault.vault_info(vault_id=param.get("vault_id")))
        ...
        return self._return_with_message(f"Finished scanning {scan_location}", action_result)
